...
export default function SalesHistory() {
    const { sales, loading }: { sales: Sale[], loading: boolean } = useStore();
    ...
    const [activeFilter, setActiveFilter] = useState<'today' | 'yesterday' | 'this_month' | 'custom' | 'all'>('all');


    const handleFilterChange = (filterType: 'today' | 'yesterday' | 'this_month' | 'custom' | 'all') => {
    ...
    const totalFilteredAmount = useMemo(() => {
        return filteredSales.reduce((sum: number, sale: Sale) => sum + (sale.total || 0), 0);
    }, [filteredSales]);
                                ...
                                <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isExpanded ? 'max-h-[1000px]' : 'max-h-0'}`}>
                                    <div className="bg-slate-50/70 p-4 border-t border-slate-200">
                                        <table className="w-full text-sm">
                                            <thead><tr className="border-b border-slate-200"><th className="p-2 text-left font-semibold text-slate-500">สินค้า</th><th className="p-2 w-24 text-right font-semibold text-slate-500 hidden sm:table-cell">ราคาหน่วย</th><th className="p-2 w-16 text-center font-semibold text-slate-500">จำนวน</th><th className="p-2 w-24 text-right font-semibold text-slate-500">รวม</th></tr></thead>
                                            <tbody>
                                                {(sale.items || []).map((item: SaleItem, index: number) => (
                                                    <tr key={index} className="odd:bg-white even:bg-transparent border-b border-slate-50 last:border-0"><td className="p-2 text-slate-700 font-medium">{item.name || 'ไม่มีชื่อสินค้า'}</td><td className="p-2 text-slate-600 text-right hidden sm:table-cell">฿{(item.price || 0).toFixed(2)}</td><td className="p-2 text-slate-600 text-center">x{item.qty || 0}</td><td className="p-2 font-semibold text-slate-800 text-right">฿{((item.price || 0) * (item.qty || 0)).toFixed(2)}</td></tr>
                                                ))}
                                            </tbody>
                                        </table>
