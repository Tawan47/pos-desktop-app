...
// Component ย่อยสำหรับ Barcode
const Barcode: React.FC<BarcodeProps> = ({ value, format = 'CODE128', width = 2, height = 50 }) => {
...
const ShippingLabel = () => {
    ...
    useEffect(() => {
        if (window.electronAPI && typeof window.electronAPI.getLogoData === 'function') {
            window.electronAPI.getLogoData().then((logoDataUrl: string) => {
                if (logoDataUrl) setShopLogo(logoDataUrl);
            });
        }
    }, []);
    
    const handleSenderChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setSender({ ...sender, [e.target.name]: e.target.value });
    const handleReceiverChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setReceiver({ ...receiver, [e.target.name]: e.target.value });
    const handleOrderChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setOrderDetails({ ...orderDetails, [e.target.name]: e.target.value });
    ...
    const handlePrint = () => {
        ...
        const codAmount = Number(orderDetails.codAmount) || 0;
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 font-sans space-y-6 sm:space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                <div>
                    <h1 className="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight">พิมพ์ใบปะหน้าพัสดุ</h1>
                    <p className="text-slate-500 font-medium mt-1">จัดการข้อมูลการจัดส่งและสร้างใบปะหน้า</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="phone" value={String(sender.phone)} onChange={handleSenderChange} className="mt-1 block w-full input-style" />
                            </div>
                        </div>
                    </div>
                    <div className="bg-white p-6 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="phone" value={String(receiver.phone)} onChange={handleReceiverChange} className="mt-1 block w-full input-style" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                     ...
                     <button
                        onClick={handlePrint}
                        className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-4 rounded-2xl transition-all shadow-xl shadow-blue-600/30 text-lg active:scale-95"
                     >
                        <i className="fa-solid fa-print mr-2"></i> สร้างและสั่งพิมพ์ใบปะหน้า
                     </button>
                     <div className="bg-white p-6 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 text-center">
                        <h3 className="text-lg font-bold text-slate-800 mb-4">ตัวอย่างบาร์โค้ดเลขพัสดุ</h3>
